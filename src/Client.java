...
public class Client extends JFrame {
	...
	public Client(String title) {
		super(title);
		this.ip=JOptionPane.showInputDialog("IP Address:");
		this.port=JOptionPane.showInputDialog("Port number:");
		this.playerName = JOptionPane.showInputDialog("What is your name?");
		connectToEchoServer1();
		runUpdateText();
		setGUI();
	}
...
//				
//				
//				Scoreboard s = new Scoreboard(playerName, score1, player2, score2, con, x);
...
//				out.flush();
				...
				firstPlayer = msg.replace("Client #" + id
						+ "\\battle first player: ", "");
	...
	public void connectToEchoServer1() {
		try {
			con = new Socket(InetAddress.getByName(this.ip), Integer.parseInt(this.port));
			out = new PrintWriter(con.getOutputStream(), true);
	...
	public void connectToEchoServer2() {
		try {
			con = new Socket(InetAddress.getByName("127.0.0.1"), 10007);
			out = new PrintWriter(con.getOutputStream(), true);
			in = new BufferedReader(new InputStreamReader(con.getInputStream()));
